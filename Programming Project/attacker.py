...
for target_index in range(len(target)):
    while True:
        ...
        for ct in range(len(CT)-1):
            target_xor_ct = chr(
                (ord(target[target_index]) ^ ord(CT[ct][target_index])))
        ...
        if (choose_ok == "y"):    # If "y", save the current state and print it out
            ...
            choose_plain = input(
                "choose a plaintext index in which you know the next character [0-9]: ")
            choose_next_character = input(
                "which char do you think is next in choosen plaintext?")
            if ((target_index+1) < len(target)):
                next_to_enter = chr((ord(target[target_index]) ^ ord(
                    CT[int(choose_plain)][target_index])))
                next_to_enter = chr(ord(next_to_enter) ^
                                    ord(choose_next_character))
